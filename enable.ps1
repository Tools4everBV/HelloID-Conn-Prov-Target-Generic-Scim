...
if (-not($dryRun -eq $true)) {
    try {
        ...
        if ( $($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or $($ex.Exception.GetType().FullName -eq 'System.Net.WebException')) {
            ...
            $errorMessage = "Could not enable scim account for: $($p.DisplayName). Error: $($errorObj.ErrorMessage)"
        } else {
            $errorMessage = "Could not enable scim account for: $($p.DisplayName). Error: $($ex.Exception.Message)"
        }
